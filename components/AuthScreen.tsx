  ...
  return (
    <div className={`min-h-full w-full flex items-center justify-center relative overflow-y-auto overflow-x-hidden transition-colors duration-500 ${isDarkMode ? 'bg-slate-900' : 'bg-sky-50'}`}>
      ...
      <div className={`backdrop-blur-md px-2 sm:px-3 md:px-5 py-3 sm:py-5 md:py-6 rounded-xl border-3 sm:border-4 md:border-5 shadow-[2.5px_2.5px_0px_0px] sm:shadow-[4px_4px_0px_0px] md:shadow-[6px_6px_0px_0px] w-[95%] sm:w-[90%] max-w-[550px] flex flex-col relative z-10 my-4 sm:my-6 md:my-8 transition-all duration-500 ${isDarkMode ? 'bg-slate-950/90 border-indigo-700 shadow-indigo-950' : 'bg-white/90 border-sky-900 shadow-[#0ea5e9]'}`}>
        ...
        {/* Header Tabs */}
        <div className={`flex w-full mb-1.5 sm:mb-2 md:mb-3 border-b-2 sm:border-b-2 md:border-b-3 transition-colors ${isDarkMode ? 'border-indigo-900' : 'border-gray-200'}`}>
            <button 
                onClick={() => setIsLogin(true)}
                className={`flex-1 font-press-start text-[0.95vmin] sm:text-[1.2vmin] md:text-[1.6vmin] py-1.5 sm:py-2 transition-colors ${isLogin ? (isDarkMode ? 'text-pink-400 border-b-2 sm:border-b-2 md:border-b-3 border-pink-400 -mb-[2px] sm:-mb-[2px] md:-mb-[3px]' : 'text-sky-600 border-b-2 sm:border-b-2 md:border-b-3 border-sky-600 -mb-[2px] sm:-mb-[2px] md:-mb-[3px]') : 'text-gray-400 hover:text-gray-600'}`}
            >
                LOGIN
            </button>
            <button 
        ...
        <h2 className={`font-press-start text-[1.2vmin] sm:text-[1.6vmin] md:text-[2vmin] text-center mb-1.5 sm:mb-2 md:mb-3 transition-colors ${isDarkMode ? 'text-indigo-100' : 'text-gray-800'}`}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-1.5 sm:gap-2 md:gap-3">
            ...
            <ArcadeButton 
              type="submit" 
              disabled={loading}
              className={`w-full mt-1 sm:mt-1.5 md:mt-2 text-[1.2vmin] sm:text-[1.5vmin] md:text-[1.6vmin] transition-all ${isDarkMode ? 'bg-pink-600 text-white border-indigo-900 shadow-indigo-900 hover:bg-pink-500' : ''} ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
                {loading ? 'LOADING...' : (isLogin ? 'START GAME' : 'REGISTER')}
            </ArcadeButton>
        </form>

        <div className="relative flex py-1.5 sm:py-2 md:py-3 items-center">
            <div className={`flex-grow border-t-2 transition-colors ${isDarkMode ? 'border-indigo-900' : 'border-gray-300'}`}></div>
            <span className={`flex-shrink-0 mx-2 sm:mx-2.5 font-press-start text-[0.65vmin] sm:text-[0.8vmin] md:text-[1vmin] transition-colors ${isDarkMode ? 'text-indigo-500' : 'text-gray-400'}`}>OR CONNECT WITH</span>
            <div className={`flex-grow border-t-2 transition-colors ${isDarkMode ? 'border-indigo-900' : 'border-gray-300'}`}></div>
        </div>
        ...
        <div className="flex flex-col gap-1 sm:gap-1.5 md:gap-2">
            ...
            >
                ...
                <span className="font-press-start text-[0.8vmin] sm:text-[1vmin] md:text-[1.2vmin] group-hover:opacity-80">GOOGLE</span>
            </button>
        </div>
        
        {/* Footer */}
        <p className={`mt-2 sm:mt-2.5 md:mt-3 text-center font-vt323 text-[1.2vmin] sm:text-[1.6vmin] md:text-[2vmin] transition-colors ${isDarkMode ? 'text-indigo-400' : 'text-gray-500'}`}>

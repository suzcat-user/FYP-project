...
const PERSONALITY_DATA: Record<PersonalityCode, PersonalityResult> = {
  [PersonalityCode.F]: {
    ...
    hobbies: [
      ...
      { name: "HIIT", description: "Short, intense intervals designed for maximum calorie burn in minimal time. It is perfect for busy schedules and delivers a big fitness boost quickly.", communityId: 7 },
  ...
  [PersonalityCode.C]: {
    ...
    hobbies: [
      { name: "Drawing", description: "Sketching ideas and scenes with pencil or pen to capture imagination. It builds observation skills, improves hand control, and is an easy way to express feelings visually.", communityId: 9 },
      { name: "Painting", description: "Creating artwork with acrylics, watercolor, or oils to explore color and texture. Each piece becomes a personal statement, and practice unlocks new creative styles.", communityId: 10 },
      { name: "Pottery", description: "Shaping clay into functional or decorative pieces using wheel or hand-building methods. The tactile process is calming, and finished work feels uniquely personal.", communityId: 11 },
      { name: "Calligraphy", description: "Stylized lettering for cards, art, or journaling that turns words into visual art. It is a relaxing craft that rewards patience and steady practice.", communityId: 12 },
      { name: "Jewellery making", description: "Designing wearable pieces with beads or wire to create custom accessories. You can experiment with colors, textures, and patterns for a signature look.", communityId: 13 },
      { name: "Guitar", description: "Learning chords and songs on acoustic or electric guitar to build rhythm and confidence. It is versatile for solo play or jamming with others.", communityId: 14 },
      { name: "Piano", description: "Playing melodies and building technique on the keys with structured practice. It develops musical understanding and lets you interpret songs with your own style.", communityId: 15 },
      { name: "Singing", description: "Vocal practice for expression, harmony, and confidence across different genres. It improves breathing control and helps you connect emotionally with music.", communityId: 16 }
    ]
  },
  [PersonalityCode.N]: {
    ...
    communities: ["Outdoor & Fitness"],
    hobbies: [
      { name: "Gardening", description: "Growing plants and creating calm, green spaces that bring daily peace. It teaches patience, connects you with seasons, and rewards you with visible progress.", communityId: 17 },
  ...
  [PersonalityCode.L]: {
    ...
    hobbies: [
      { name: "Cooking", description: "Trying recipes and building everyday kitchen skills you can rely on. It brings comfort through routine while giving space to experiment with flavors.", communityId: 26 },
      { name: "Baking", description: "Creating breads and desserts with relaxing precision and a cozy payoff. Measuring, mixing, and timing make it a satisfying and mindful process.", communityId: 27 },
...
const normalizeHobbyName = (name: string) => name.trim().toLowerCase();

export const getHobbyByName = (name: string): Hobby | undefined => {
  const key = normalizeHobbyName(name);
  const allHobbies = Object.values(PERSONALITY_DATA).flatMap(result => result.hobbies);
  return allHobbies.find(hobby => normalizeHobbyName(hobby.name) === key);
};
